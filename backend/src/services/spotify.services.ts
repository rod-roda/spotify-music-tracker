...
import type { SpotifyTokenResponse, SpotifyProfile, TopTracks, TopArtists, RefreshTokenResponse } from "../schemas/spotify.schema";
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response?.status === 429) {
                ...
                if (attempt < maxRetries) {
                    process.stderr.write(`[spotify] Rate limit atingido. Aguardando ${waitTime}ms (tentativa ${attempt + 1}/${maxRetries})\n`);
...
export async function requestTokenRefresh(refreshToken: string): Promise<RefreshTokenResponse>
{
    return spotifyApiRequest(async () => {
        const response = await axios.post(
            'https://accounts.spotify.com/api/token',
